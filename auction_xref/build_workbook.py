...
def main():
    ...
    lines = [
     ...
     ("Source of the October catalogue", True),
     (f"Fetched directly from auctionhouselondon.co.uk/current-auction on 16 September 2026 and",False),
     (f"parsed from the page itself, so this is the complete sale as listed: {len(catalogue)} lots.",False),
     ("An earlier version of this workbook used a browser copy-paste that had silently dropped",False),
     ("lots, and wrongly reported several as not being in the October catalogue. That is fixed.",False),
     ("The sale was still taking entries, so it will grow before 7-8 October. Re-run nearer the",False),
     ("date: python3 build_workbook.py rebuilds everything from source.",False), ("", False),
     ("Lot numbers", True),
     ("'Oct lot' holds the site's own reference for the lot, taken from the page. The listing page",False),
     ("does not print auction lot numbers, so check the catalogue for the number to bid on.",False),("",False),
     ("What this is", True),
     ("Three sheets:", False),
